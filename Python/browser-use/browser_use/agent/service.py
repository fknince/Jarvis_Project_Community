...
class Agent:
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		elif self.model_name == 'nisus-ai-chat':  # NisusLLM kullanılıyorsa
			try:
				prompt = "\n".join([msg.content for msg in converted_input_messages if isinstance(msg, BaseMessage)])
				response_text = self.llm._call_nisus_api(prompt)  # Nisus LLM API çağrısı
				parsed_json = self.message_manager.extract_json_from_model_output(
					response_text)  # JSON formatına çevirme
				parsed = self.AgentOutput(**parsed_json)
			except (ValueError, ValidationError) as e:
				logger.warning(f'Failed to parse NisusLLM output: {response_text} {str(e)}')
				raise ValueError('Could not parse response from NisusLLM.')
		...
		print(f"Agent Input Messages: {input_messages}")
